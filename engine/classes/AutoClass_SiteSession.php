...
class SiteSession
{
	...
	public function write($id, $data)
	{
		...
		if (isset($_SESSION['userID'])) {
			$oldID = apc_fetch($this->_sessionName . '/USERS/' . $_SESSION['userID']);
			if ($oldID != $id) {
				$this->destroy($oldID);
				apc_store($this->_sessionName . '/USERS/' . $_SESSION['userID'], $id);
			}
		}
		return $success;
	}
	...
	{
		self::create();
		if (!is_null(self::$instance) && isset($_SESSION['userID'])) {
			return true;
		} else {
			return false;
		}
